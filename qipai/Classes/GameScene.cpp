...
bool GameScene::init(){
	...
	do 
	{
		...
		//地主标签
		Dictionary * strings = Dictionary::createWithContentsOfFile("res/strings.xml");
...
Poker* GameScene::selectPoker(int huaSe,int num){
	Poker* pk;
	if(huaSe != Gui)
		pk = Poker::create("res/poker.png",CCRect(num*pkWidth,huaSe*pkHeight,pkWidth,pkHeight));
	else
		pk = Poker::create("res/poker.png",CCRect((num-XiaoGui)*pkWidth,huaSe*pkHeight,pkWidth,pkHeight));
	pk->setHuaSe(huaSe);
	pk->setNum(num);
	pk->setGameMain(this);
	return pk;
