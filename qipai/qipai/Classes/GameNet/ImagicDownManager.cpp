...
void ImagicDownManager::OnImagic()
{
	...
	if (response->getHttpRequest() == NULL)
	{
		return;
	}

	if (response->getHttpRequest()->getUrl() == NULL)
	{
		return;
	}
...
void ImagicDownManager::addDown(cocos2d::Node* pNode,std::string kUrl,std::string kFileName,bool bSameSize )
{
	...
	if (!bHaveGet)
	{
		cocos2d::network::HttpRequest* pRequest = MCWebReq::instance().getFileEx(kUrl,
